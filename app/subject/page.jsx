...
export default function SubjectsPage() {
  ...
  useEffect(() => {
    fetch("/api/subject") // ✅ Change to "subjects" (plural)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    const res = await fetch("/api/subject", { // ✅ Change to "subjects"
      method: "POST",
  ...
  const handleDelete = async (id) => {
    const res = await fetch(`/api/subject/${id}`, { // ✅ Change to "subjects"
      method: "DELETE",
    });
