...
export default function UpdateParentForm() {
  const { formData, handleChange, message, setMessage } = useParentForm();

  const handleSubmit = async (e) => {
    e.preventDefault();
    setMessage("");

    const response = await fetch("/api/registrar/parent", {
      method: "PUT",
    ...
    setMessage(response.ok ? "Parent updated successfully" : data.error || "Update failed");
  };
