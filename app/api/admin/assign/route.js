...
export async function GET() {
  try {
    ...
    const groupedAssignments = assignments.reduce((acc, assignment) => {
      ...
      acc[staffKey].sections.push({ id: section.id, name: section.section });

      return acc;
    }, {});

    return new Response(JSON.stringify(Object.values(groupedAssignments)), {
...
export async function POST(req) {
  try {
    ...
    if (!staffID || !gradeId || !subjectId || !Array.isArray(sectionIds) || sectionIds.length === 0) {
    ...
    return new Response(JSON.stringify({ message: 'Assignment created successfully' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
...
export async function DELETE(req) {
  try {
    ...
    return new Response(JSON.stringify({ message: "All assignments deleted for this staff member" }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
