...
export default function StudentList() {
  ...
  const fetchStudents = async (e) => {
    ...
    try {
      const res = await fetch(
        `/api/head/student/list?year=${year}&grade=${grade}&section=${section}`
      );
  ...
  const handleStatusChange = async (studentID, newStatus) => {
    try {
      const res = await fetch('/api/head/student/status', {
        method: 'PUT',
        headers: {
          ...
          <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
            <thead className="bg-blue-100">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-sm font-semibold text-blue-800">MiddleName</th>
                <th className="px-4 py-2 text-left text-sm font-semibold text-blue-800">LastName</th>
                <th className="px-4 py-2 text-left text-sm font-semibold text-blue-800">Email</th>
